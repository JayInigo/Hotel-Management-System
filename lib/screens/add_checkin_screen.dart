...
class _AddCheckInScreenState extends State<AddCheckInScreen> {
  ...
  final _guestStatusController = TextEditingController();
  ...
  final String _groupName = 'YourGroupName';
  ...
  try {
    ...
    if (permission == LocationPermission.denied ||
      ...
      _showSnack('Location permission denied. Please allow in browser.');
      return;
    }
  ...
  try {
    final docRef =
        FirebaseFirestore.instance.collection('hotel_checkins').doc();
    final storageRef = FirebaseStorage.instance
        .ref()
        .child('checkins/${docRef.id}.jpg');

    // Upload bytes instead of File (works on web!)
    ...
    await docRef.set({
      ...
      'guestStatus': _guestStatusController.text.trim(),
      'photoUrl': photoUrl,
      'lat': _lat,
      'lng': _lng,
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInput(_businessController, 'e.g. Grand Hotel Manila'),
            const SizedBox(height: 16),

            _sectionLabel('Note'),
            _buildInput(_noteController, 'e.g. Visited client at lobby'),
            const SizedBox(height: 16),

            _sectionLabel('Room Type'),
            _buildInput(_roomTypeController, 'e.g. Deluxe, Suite, Standard'),
            const SizedBox(height: 16),

            _sectionLabel('Guest Status'),
            _buildInput(_guestStatusController,
                'e.g. Check-In, Check-Out, Staying'),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(8),
                border: Border.all(
                  color: _lat != null ? Colors.green : Colors.grey[300]!,
                ),
              ),
              child: Text(
                _lat != null
                ...
                style: TextStyle(
                  color: _lat != null ? Colors.green[800] : Colors.grey,
